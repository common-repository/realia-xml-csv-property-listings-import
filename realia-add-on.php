...
function realia_import($post_id, $data, $import_options, $article) {
	...
	if ( realia_version() < 4 ) {
		...
		if ( $geocoding_failed ) {
			...
			$realia_addon->log( "WARNING Geocoding failed with status: " . $details['status'] );
			if ( array_key_exists( 'error_message', $details ) ) {
				$realia_addon->log( "WARNING Geocoding error message: " . $details['error_message'] );
			}
		}
